...
const EditCategory = ({ data }) => {
  ...
  React.useEffect(() => {
    if (!data?._id) return
    setFormFields({ name: data.name || '', images: Array.isArray(data.images) ? data.images : [] })
  ...
  const handleSubmit = async e => {
    ...
    try {
      setIsLoading(true)
      const response = await putData(`/api/category/${data._id}`, { name: formFields.name, images: formFields.images }, { withCredentials: true })
    ...
    finally { setIsLoading(false); context.setIsOpenFullScreenPanel({ open: false, model: null, data: null }) }
  }
    ...
    <section className='p-5 min-h-screen' style={{ background: C.bg }}>
      ...
      <form onSubmit={handleSubmit} className='relative z-10 max-w-2xl mx-auto flex flex-col gap-5'>
        <div className='flex items-center gap-3 mt-2'>
          ...
          <div>
            <h1 className='text-[20px] font-[900] tracking-tight' style={{ color: C.text }}>Edit Category</h1>
            <p className='text-[12px]' style={{ color: C.muted }}>Update category details</p>
          </div>
        </div>
          ...
          <div className='grid grid-cols-2 sm:grid-cols-4 gap-3'>
            ...
            <UploadBox setPreview={setPreview} multiple={false} name='images'
              ...
              onUploadSuccess={(imgs, allowMultiple) => {
                setFormFields(prev => ({ ...prev, images: allowMultiple ? [...prev.images, ...imgs] : imgs }))
                setPreview(allowMultiple ? prev => [...prev, ...imgs] : imgs)
              }} />
          </div>
        </div>
        ...
        <button type='submit' disabled={isLoading}
          ...
          {isLoading ? <><CircularProgress size={20} color='inherit' /> Saving…</> : 'Save Changes'}
        </button>
      </form>
    </section>
  )
